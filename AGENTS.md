...
- `home/h82/`: Home Manager modules by domain, each with its own `default.nix` that `home/h82/default.nix` imports -- `agents/` (Claude Code, Gemini CLI, agent plugins), `desktop/` (Fcitx5, terminal, `kde/`), `dev/` (Git, containers, Android SDK), `security/` (GPG, SSH), `shell/` (Zsh/shell config).
