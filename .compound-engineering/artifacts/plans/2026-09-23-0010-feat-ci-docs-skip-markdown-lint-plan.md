...
- KTD3. **Give `markdown-lint` its own CI job, separate from `flake-check`, gated to the complement of flake-check's condition.** Acceptance criteria require lint coverage on docs-only PRs, but those PRs are exactly the ones that skip `flake-check` — the job `nix flake check` normally runs inside. A check registered in `checks.${system}` is not itself "run on every PR" unless something invokes it outside the conditional job. The job runs only when `flake-check` is skipped (a genuine docs-only PR); every other event already gets it via `flake-check`'s own `nix flake check`, so `markdown-lint` is exercised exactly once per event rather than twice.
...
- KTD6. **Default markdown-lint rule set disables MD013 (line length), MD033 (inline HTML), MD041 (first-line heading), and MD036 (emphasis used as heading), and reconfigures MD025 (single top-level heading) to ignore frontmatter titles.** Verified directly: `docs/verification.md` and `docs/provisioning.md` contain lines up to 8748/818 characters (no hard-wrap convention); most sampled `docs/`, plan, and solution files contain inline HTML tags; `CLAUDE.md`'s entire content is a single `@AGENTS.md` import line with no heading; this repo's plan documents use bold text as sub-headings to group requirements by concern (an established `ce-plan` convention, not a defect); and MD025's default `front_matter_title` option treats every plan's YAML `title:` field as a competing top-level heading against its matching `# H1`, which every unified plan in this repo pairs by design. Enabling these rules by default would fail the check on existing, intentional content on day one. This exception list grew beyond what was estimated during planning (see Assumptions) once the check ran against the full corpus; the additions are rule-level exceptions, not content rewrites, consistent with the Stop condition above.
