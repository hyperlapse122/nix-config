...
Claude Code rewrites `~/.claude/settings.json` itself, so those defaults live in the managed settings tier instead. That tier outranks user and project settings, so a model or effort chosen at runtime does not persist across sessions. To hand either choice back to the user, remove the corresponding key from `modules/nixos/claude.nix`. The `opus[1m]` value starts every session on the 1M-context variant; environments where 1M context is unavailable fall back to the standard context. Drop the `[1m]` suffix to default to the standard context instead.
