...
1Password and Kleopatra now start at login from `home/h82/desktop/kde/autostart.nix`, so neither needs a manual launch. Home Manager owns `~/.config/autostart/1password.desktop` and `~/.config/autostart/kleopatra.desktop` and overwrites them on activation, which makes the apps' own "start at login" toggles inert. Change the module, not the desktop entries.
...
The `opus[1m]` value starts every session on the 1M-context variant of the `opus` alias, which follows the current Opus model; environments where 1M context is unavailable fall back to the standard context. Drop the `[1m]` suffix to default to the standard context instead. The `medium` value is the declared reasoning effort for whichever model serves the session.
