...
let
  ...
  enrollLuksTpm2 = pkgs.writeShellApplication {
    ...
    text = ''
      ...
      Enroll the configured LUKS2 device for TPM2 unlock.
...
in
{
  options.my.system.boot.tpm-luks-enroll = {
    enable = lib.mkEnableOption "manual TPM2 enrollment command for a LUKS2 device";

    device = lib.mkOption {
      type = lib.types.str;
      default = "";
      example = "/dev/disk/by-uuid/65f6a6d1-31e0-4bf5-bf13-cbc7705a1163";
      description = "LUKS2 block device to enroll with systemd-cryptenroll.";
    };
    ...
    assertions = [
      {
        ...
        message = "Hosts with my.system.boot.tpm-luks-enroll.enable = true must set my.system.boot.tpm-luks-enroll.device.";
      }
    ];
